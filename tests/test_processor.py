...
def test_alternative_endings():
    ...
    tokens = [
        "artist", "downtune:0", "tempo:90", "start",  # Header
        "new_measure",  # Measure 0: Intro
        "note:s1:f0:E5", "wait:480",
        "new_measure",  # Measure 1: Verse start (repeat open)
        "measure:repeat_open", "note:s1:f2:F5", "wait:480",
        "new_measure",  # Measure 2: Verse middle
        "note:s1:f4:G5", "wait:480",
        "new_measure",  # Measure 3: Verse end (repeat close)
        "measure:repeat_close:2", "note:s1:f5:A5", "wait:480",
        "new_measure",  # Measure 4: Chorus start (repeat open)
        "measure:repeat_open", "note:s2:f0:B4", "wait:480",
        "new_measure",  # Measure 5: Chorus middle
        "note:s2:f2:C5", "wait:480",
        "new_measure",  # Measure 6: First ending (alternative 1)
        "measure:repeat_alternative:1", "note:s2:f4:D5", "wait:480",
        "new_measure",  # Measure 7: Second ending (alternative 2)
        "measure:repeat_alternative:2", "note:s2:f5:E5", "wait:480",
        "new_measure",  # Measure 8: Chorus end (repeat close)
        "measure:repeat_close:2", "note:s2:f7:F5", "wait:480",
        "new_measure",  # Measure 9: Outro
        "note:s1:f0:E5", "wait:960",
        "end"
    ]
    ...
    assert len(measures) == 11  # Header + 9 musical measures + end token in last measure
    ...
    assert opens == [1, 4]  # Repeats start at measures 1 and 4 (indices 1 and 4 in token_measures)
    assert closes == [3, 8]  # Repeats end at measures 3 and 8 (indices 3 and 8 in token_measures)
    assert alternatives == [6, 7]  # Alternatives at measures 6 and 7 (indices 6 and 7 in token_measures)
    ...
    expected_order = [
        0,         # Intro
        1, 2, 3,   # Verse first time (measures 1-3)
        1, 2, 3,   # Verse second time (repeat)
        4, 5, 6,   # Chorus first time (goes to alternative 1)
        4, 5, 7,   # Chorus second time (goes to alternative 2)
        8,         # Chorus end (repeat close)
...
def test_complex_alternative_endings():
    ...
    tokens = [
        ...
        "measure:repeat_alternative:3", "note:s2:f7:F5", "wait:480",
        "new_measure",  # Measure 9: Section B end (repeat close)
        "measure:repeat_close:3", "note:s2:f9:G5", "wait:480",
    ...
    expected_order = [
        ...
        1, 2, 3,   # Section A first time
        1, 2, 3,   # Section A second time
        4, 5, 6,   # Section B first time (alt1)
        4, 5, 7,   # Section B second time (alt2)
        4, 5, 8,   # Section B third time (alt3)
        9,         # Section B end
        10         # Final section
    ]
    ...
    section_b_plays = [measure_play_order[7:10], measure_play_order[10:13], measure_play_order[13:16]]
...
def test_alternative_endings_edge_cases():
    ...
    tokens = [
        "artist", "start",
        "new_measure",
        "measure:repeat_alternative:1", "note:s1:f0:E5", "wait:480",
        "new_measure",
        "note:s1:f2:F5", "wait:480",
        "end"
    ]
    ...
    tokens = [
        "artist", "start",
        "new_measure",
        "measure:repeat_open", "note:s1:f0:E5", "wait:480",
        "new_measure",
        "measure:repeat_alternative:1", "measure:repeat_alternative:2", "note:s1:f2:F5", "wait:480",
        "new_measure",
        "measure:repeat_close:2", "note:s1:f4:G5", "wait:480",
        "end"
    ]
