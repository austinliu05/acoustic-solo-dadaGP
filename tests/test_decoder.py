...
def test_encode_decode(gp_path):
    ...
    print("Song parsed successfully.")
    tokens = guitarpro2tokens(read_song, "", True, False)

    processed_tokens = tracks_check(tokens)

    tokens_for_decoding, tunings = pre_decoding_processing(processed_tokens)
    print("Pre-decoding processing done.")
    decoded_song = tokens2guitarpro(tokens_for_decoding, True, tunings)
    ...
    encoded_tokens = guitarpro2tokens(decoded_song, "", True, False)
    ...
    #
    #
    #     encoded_tokens = guitarpro2tokens(decoded_song, "")
    #     assert tokens == encoded_tokens
    #
    # read_song = gp.parse(gp_path)
